...
1. **Confirm the five rate-limit namespace ids** in `wrangler.toml` (per-class: AUTH 10, GAME 30, ME 10, ADMIN 20, LEADERBOARD 100 req/min, period 60). Per the CURRENT Cloudflare docs (S1k-C/S1k-D), `namespace_id` is a **string containing a positive integer that you define** — the docs' own example is `"1001"`. The ids `1001`–`1005` ARE this project's namespace ids (`1001`–`1004` pre-date the leaderboard work and are already working; `1005` is the new leaderboard class) — there is **no placeholder policy** and no dashboard provisioning (bindings are not visible in the dashboard). The single account-level caveat: confirm no OTHER rate-limit binding on the Cloudflare account already uses any of `1001`–`1005` — a shared id intentionally shares counters across bindings/Workers (allowed only with uniform limits, S1b; deploy Gate 3 enforces that consistency).
