...
1. **Choose the five rate-limit namespace ids** and REPLACE the reserved placeholders `namespace_id = "1001"…"1005"` in `wrangler.toml` (per-class: AUTH 10, GAME 30, ME 10, ADMIN 20, LEADERBOARD 100 req/min, period 60). Per the CURRENT Cloudflare docs (S1k-C), `namespace_id` is a **string containing a positive integer that you define** — uniquely across your account's rate-limit bindings (bindings sharing an id share counters, even across Workers). The docs' own example is `"1001"`, so the values are format-valid — the gate rejects 1001–1005 only because THIS project reserves them as placeholders (deploy.yml Gate 3, `bun ./scripts/check-rate-limit-config.ts`). There is **no dashboard provisioning step** for these bindings (they are not visible in the dashboard) — pick five distinct integers (e.g. 2001–2005), confirm no other Worker on the account already uses them, and paste them in. A single shared id across all bindings with uniform limits is possible (S1b).
2. **Edge read-limit rule (operational, NOT code — plan §F.2)**: per-IP 100 req/min on `GET /api/*` via a Cloudflare dashboard rule. The authenticated `/api/leaderboard/*` reads are additionally app-limited per-user since S1k — the edge rule remains the layer for unauthenticated/other public GETs.
