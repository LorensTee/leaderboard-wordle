...
Post-handoff fixes:
- **`929ec44 fix(phase3): worker patch defers on the client-build closeBundle (fresh checkout)`** — the first GitHub Actions `unit-and-build` run failed because vite 8 fires `closeBundle` once per build environment and the client phase runs before the adapter writes `_worker.js`; `patchWorker` now defers (with a U5 regression test) and `vite.config.ts` imports carry the `.ts` extension.
- **I5c rewrite (integration job failure #2)** — the concurrent sweep + guess test asserted "≥1 finalization" from two sweeps racing an expired-puzzle guess; under SKIP LOCKED both sweeps may legitimately skip the row while the guess holds the lock. Now asserted deterministically: sentinel-pinned skip behavior, the guess never succeeds, ≤1 real finalization, and self-healing (the next sweep finalizes the row). Re-run 6× plus the full 55-test suite.
- **Batched month fixtures (integration job failure #3)** — I9 (and I11/I12) seeded up to 30 days with per-day inserts (~60+ Neon round trips, scaling with the calendar); CI's cross-region latency blew the 30s test budget. `seedFinalizedDays`/`insertGamesBatched` now use one parameterized multi-row INSERT per table (I9: 27s → <2s; full suite 314s → 58s); `testTimeout` raised to 60s with a documented rationale.
- **Correctness audit (2026-08-30, two concerns resolved, gates re-run on the final tree)** — Concern A (sweep `FOR UPDATE SKIP LOCKED` scope): the mechanism matches the plan's authoritative §6 contract (soft selection filter + per-row own-transaction `finalizePuzzle` as the authoritative guard); comments rewritten to state the precise semantics and integration test **I5d** pins the concurrent-sweep guarantee. Concern B (swallowed cron failures): `scheduled()` now logs AND rethrows so failed settlements are marked FAILED in the Cloudflare dashboard (recorded deviation from plan §7.3's literal wording); pinned by a DB-free unit test. Full gates after the audit: lint clean, check 0 errors, unit 161, integration 56/56 (live Neon, incl. I5d ×5).

Both causes + fixes are recorded in the Phase 3 implementation deviations table in `docs/contradictions-and-gaps.md`.