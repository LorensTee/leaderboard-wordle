...
The admin account associated with the project owner's Google identity should be provisioned with `admin` role. Authorization checks must use the role/identity model rather than scattering email comparisons throughout routes.
...
3. SvelteKit owns pages/routing; Hono owns `/api/*`.
4. Zod and Drizzle-Zod solve different validation problems and should be used together.
5. Use TanStack's Svelte integrations where they provide real value.
6. Keep future answers server-side; a public repository may contain the valid-guess dictionary.
7. Store raw game facts so ranking rules can evolve without rewriting history.
8. Keep roles and authorization explicit from the start.
9. Treat security verification as an ongoing process using requirements, tests, scanning, dependency checks, and manual attack exercises.
10. Avoid unnecessary dependencies and features until they have a concrete role.
11. Keep platform-specific deployment code at the application edges.
