...
import { withDbScope } from '$server/db/memo';
import {
	HSTS_HEADER_VALUE,
...
export const handle: Handle = ({ event, resolve }) =>
	...
	// client is request-bound in the Workers runtime (a module-level memo is
	// rejected with "Cannot perform I/O on behalf of a different request"),
	// so every request runs inside withDbScope: getDb/getAuth share ONE
	// client within the request, and it is closed when the request finishes.
	// Wrapping the whole handle — not just resolve — also covers this
	// hook's own session resolution (fast path stays DB-free).
	withDbScope(async () => {
