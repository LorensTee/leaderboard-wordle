...
- **Test-count snapshot (corrected 2026-08-23, updated B8):** per-commit suite totals (unit + integration): B6 = 14+7 = **21**; B7 (`c228196`) = 18+7 = **25** (the "25/25" figure referred to this pre-fix run); `c90716f` = 21+7 = **28**; final (`4b83167`+ — one unit test added: the deployed-Worker secret regression) = 22+7 = **29**. The "21 unit + 7 integration = 28 total" wording corresponded to the `c90716f` snapshot. **B8 (2026-08-24):** unit **33** (added hono-auth 7 + build-word-list 4) + integration **8** (added account token-column test) = **41**, e2e 1.
...
### Phase 0 B8 — final close-out audit (2026-08-24)

Final Phase-0 completion run against the tracked HEAD `7f13ddd` (the DeepSeek Phase-0 close-out prompt). All gates re-verified on the actual repository; the remaining audit gaps closed:

- **Hono-side authentication helper — implemented** (`src/server/middleware/auth.ts`, the Architecture §1300 Phase-0 item that was still missing): `authContext` middleware resolves the Better Auth session from request cookies/headers (fast-path: no session cookie → `null` without a DB round-trip — mirrors `hooks.server.ts`), `requireAuth` rejects unauthenticated requests with the NG21 `UNAUTHORIZED` envelope, and the typed context value is `c.get('auth')` → `{ session, user } | null` (`AuthContext` added to `HonoVariables`). Mounted in `routes.ts` on `*` (resolution) and on `/api/game/*`, `/api/me/*`, `/api/admin/*` (guard, Phase-1+ namespaces). `/api/auth/*` is untouched — Better Auth keeps owning its endpoints (unit-tested). Never reads `event.locals`; the bridge passes only request/platform/ctx, so Hono authentication is fully independent. Tests: `tests/unit/hono-auth.test.ts` (7) covering the four required contracts — authenticated resolution, missing/invalid rejection (incl. bad-signature cookie → 401 DB-free: verified better-auth 1.7.1 returns `null` before any session lookup), fast-path is DB-free, bridge ignores populated `event.locals`, and `/api/auth/get-session` still answers 200 `null`.
- **Lint gate (NG23 Phase-0 item — was missing):** ESLint 9 flat config (`eslint.config.js`) + `typescript-eslint` + `eslint-plugin-svelte`; script `bun run lint`; CI step before `check`. `svelte-check` untouched. Generated artifacts excluded (`auth-schema.generated.ts`, `worker-configuration.d.ts`, build/tool output). Verified to fail on a genuine violation and pass the whole tree.
- **Better Auth CLI pinned (was `auth@latest`):** devDependency `auth@1.7.1` → `bun.lock`; `auth:schema` now runs the local binary. Regeneration deterministic: two consecutive runs byte-identical AND byte-identical to the committed canonical file. `auth:check` added as a CI gate.
- **Google token-storage contradiction — resolved as a documented decision** (see §Session/cookie strategy): the `account` token columns are Better Auth-managed core-schema fields; its 1.7.1 OAuth callback persists `access_token`/`id_token`/scope/expiries on every sign-in (no configuration can disable it; removing the columns breaks account creation and CLI regeneration). The app requests only OIDC default scopes and never `access_type=offline` → Google returns no refresh token → `refresh_token` stays NULL (integration-regression-tested: columns exist and are nullable). No Google-API code exists in `src/` (verified by grep). Token storage is Better Auth behavior, not application behavior.
- **CI final policy — integration gate is MANDATORY:** the `integration` job now FAILS explicitly when the non-production `DATABASE_URL` secret is missing instead of silently skipping DB verification (fork PRs without the secret fail loudly; see ci.yml). Lint and `auth:check` are steps in `unit-and-build`. `types:check` stays after the build (wrangler 4.125.0 mainModule emission is build-state-dependent); its hermeticity was re-proven: with `.env`/`.dev.vars` absent (CI condition), `wrangler types` regenerates byte-identical to the committed `worker-configuration.d.ts`; locally the files intentionally fold into `Env` (documented, never committed).
- **Word-list pipeline hardened:** `scripts/build-word-list.ts` now FAILS on duplicates (previously silently deduped — a mismatch with its documented rule); parse/build logic extracted as pure functions and unit-tested (`tests/unit/build-word-list.test.ts`, 4 tests: valid deterministic parse, invalid entries rejected, duplicates rejected, comments/blank lines ignored). Artifact builds byte-identical across repeated runs. The 20-word list remains an explicitly labeled Phase-0 fixture (sample data) pending the provenance-disciplined real import (NG16, Phase 1).
- **Counts (B8):** unit **33** / integration **8** / e2e **1**; total 41 + 1 = 42 tests (previous snapshot 22+7+1 = 30).
- **Post-B8 CI fix (2026-08-24, `690c6ec`):** the first CI run of the new workflow failed at `bun run auth:check` — `cp: cannot create regular file '.cache/auth-check.before.ts'`: `.cache/` is gitignored and absent on a fresh runner. `auth:check` now self-creates it (`mkdir -p .cache && cp …`); reproduced in a clean clone, fixed (parity OK), and the full unit-and-build sequence re-verified green in a fresh runner-shaped clone (see commit message).
- **Environment limitations recorded (not failures):** live Google OAuth re-run requires interactive browser + Google consent — the B7 gate (2026-08-23) remains the evidence; the account row it produced was subsequently cascade-deleted by the integration TRUNCATE fixture (verified in the live DB), which is expected. Neon integration suite re-run green on 2026-08-24 (see B7 table below, updated).
...
Do not request, use, or read Google access/refresh tokens — the application only needs user identity from Google OIDC, not API access to Google services. **Resolved decision (Phase 0 B8, 2026-08-24):** the `account`-table token columns (`access_token`/`refresh_token`/`id_token`/expiries/`scope`) are Better Auth MANAGED core-schema fields that its 1.7.1 OAuth callback writes on every sign-in — they cannot be removed or configured away without breaking account creation and CLI-schema regeneration. The application therefore: (a) requests only the OIDC default scopes (`email`, `profile`, `openid` — no `access_type=offline`, so Google returns no refresh token and `refresh_token` stays NULL), and (b) never reads the stored values (no Google API code exists in `src/`). The columns remain nullable and inert; token storage is Better Auth behavior, not application behavior. Regression guard: `tests/integration/db.test.ts` asserts the columns exist and are nullable.
...
The Phase-0 Hono authentication helper (`src/server/middleware/auth.ts`) is the single mechanism application routes use for this:

- `authContext` — resolves the Better Auth session from the raw request cookies/headers on every API request (cookie fast-path: no `better-auth.session_token` cookie → `null` without a DB round-trip). Sets the typed `auth` variable (`c.get('auth')` → `{ session, user } | null`).
- `requireAuth` — rejects unauthenticated requests with the standard `UNAUTHORIZED` error envelope (NG21) before any handler runs. Mounted on the protected namespaces `/api/game/*`, `/api/me/*`, `/api/admin/*` in `routes.ts`.
- It never reads SvelteKit `event.locals` (the bridge does not pass them), never creates a second session system, and does not apply to `/api/auth/*` (Better Auth owns those endpoints).
