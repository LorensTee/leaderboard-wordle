...
| 15 | CI | ✅ PASS (2/3 jobs) / ⏳ integration | run 3 (`607d00d`, 2026-08-23): `unit-and-build` **all 10 steps green** (incl. post-build types:check — the build-state-dependent order, and verify:bundle); `e2e` **green** (Playwright chromium smoke through SvelteKit→Hono chain); `integration` ❌ step "Apply migrations" failed in ~1 s (step gate worked — secret present, step ran). Local reproduction of the exact job env (only `DATABASE_URL`, no `.env`) applies the migration successfully → failure is runner→Neon environmental (likely Neon IP allowlist or secret mismatch; runner logs required repo-admin rights, unavailable anonymously). Fix pending user-provided step log / Neon IP-Allow settings |
